...
//
// 把 ~/.dsh/skills/<name>/SKILL.md 一键注册为Agent：
//   - frontmatter 的 name/description 进Agent id/name/triggers
//   - 正文（去掉 frontmatter）作为Agent systemPrompt
//   - routes 留空（继承父会话模型），用户可在 Settings 页或
//     agents.json 里补
//
// 只做提示词搬运，不搬 skill 的工具依赖：Agent 子代理全量继承主会话
...
/**
 * 把一个 skill 转成Agent 定义（不落盘；由 registry.upsert 持久化）。
 *
 * @param {string} skillsRoot skills 根目录
 * @param {string} skillDir   skill 目录名
 * @returns {{agent: object, warnings: string[]}}
 */
export function skillToAgent(skillsRoot, skillDir) {
  ...
  if (!body.trim()) throw new Error(`skill ${skillDir} 正文为空，不适合作为Agent 提示词`)
  ...
  const systemPrompt = [
    '你是一位被委派的Agent 子代理，收到的是完整独立任务描述。',
    '',
    body,
  ].join('\n')
  return {
    agent: { id, name, emoji: '📥', triggers, systemPrompt, routes: [], enabled: true },
    warnings,
  }
}
