...
import { isUnspecifiedSetting } from './agents.js'
...
/**
 * v1.11.0：规范化任务文本用于去重键计算。
 * 自动重试会往任务文本加「【前情提示】…」「【全新会话执行指令】…」等前缀块，
 * 重试文本 = 前缀 + 原文；hash 前必须剥离这些前缀块（识别到行尾/空行），
 * 否则原任务与重试任务 hash 不一致、去重失效。
 *
 * 规则：
 *   1. 以「【…】」起首的行（整个 block 标记）到下一个空行或非前缀行，均视为前缀；
 *   2. 前缀后的空行也一并跳过（前缀与正文间的分隔）；
 *   3. 剩余文本 = 去重用原文。
 */
const PREFIX_BLOCK_RE = /^【[^】]*】/
export function normalizeTaskForDedup(task) {
  const raw = String(task ?? '')
  const lines = raw.split('\n')
  let i = 0
  while (i < lines.length) {
    if (PREFIX_BLOCK_RE.test(lines[i].trim())) {
      i++
...
export function dedupKey(agentId, task) {
  const normalized = normalizeTaskForDedup(task)
  const hash = createHash('sha256').update(normalized).digest('hex')
  return `${agentId}::${hash}`
}

/**
 * v1.11.10：构造"自动换档重试"的任务前缀。
 *
 * 缺陷根因（用户现场实证）：旧实现在前缀里硬编码「${failoverFrom}（ACP 产品）」，
 * 于是当 fallback 链走到【末档】时——末档通常是宿主侧 spawn 路由（deepseek-official
 * 等 LLM 路由，没有 ACP 产品会话）——spawn child 读到"ACP 产品"语境后仍去调用
 * product_submit，必然报 `no remote product session is bound to this agent
 * (recovery failed)`，白烧一轮。
 * 修复：文案按【来源档】与【目标档】各自的真实形态生成；目标档非 ACP 时追加明确的
 * 执行模式说明，物理上劝止中继工具。
 *
 * @param {string} taskText 已包装的任务文本（buildTaskText 输出）
 * @param {object} p
 * @param {string} p.failoverFrom     上一档 provider 名（缺省则不生成前缀）
 * @param {string} [p.failoverError]  上一档失败原因
 * @param {boolean} p.fromAcp         来源档是否为 ACP 产品
 * @param {boolean} p.targetAcp       目标档（即将新建的 child）是否为 ACP 产品
 * @returns {string}
 */
export function buildFailoverTaskText(taskText, { failoverFrom, failoverError, fromAcp, targetAcp }) {
  if (!failoverFrom) return taskText
  const prev = failoverError ? String(failoverError).replace(/\s+/g, ' ').slice(0, 500) : '执行失败'
  const fromDesc = fromAcp ? `${failoverFrom}（ACP 产品）` : `${failoverFrom}（模型路由）`
  const modeHint = targetAcp
    ? ''
    : '\n\n【执行模式】你是宿主侧普通子代理，本任务没有绑定任何 ACP 产品会话：不要调用 product_submit 或任何产品中继工具，直接用你自己的工具完成任务并回报结果。'
  return `【前情提示】此任务此前尝试由 ${fromDesc} 执行，${prev}。请以全新会话执行本任务（注意：此前的中间产物可能不存在或不可靠，请自行校验后重新完成）。${modeHint}\n\n${taskText}`
}

/**
 * v1.11.12：ACP 路由「填了可能没生效」的派发期预检（纯函数，便于单测）。
 *
 * 背景：agents.json 可手工编辑，provider 对但 model/effort 写错（拼错大小写、
 * 用了已下线的模型等）。产品侧对非法取值的语义是【回退自己的默认值继续执行】，
 * 而不是报错——所以派发链路本身不会失败，用户看到的是"配了没生效"。本函数在
 * 【派发前】做一次目录预检，把这种静默降级变成日志里可见的一行注记（只记请求值与
 * "是否在目录中"，不记任务内容）；产品侧真正拒绝时的实际生效值由
 * logConfigOptionError 走 `config-option-error` 事件补记，两者互为前后验证。
 *
 * 判据刻意保守：目录缺失 / 该 provider 无条目 / 取值列表为空（探测失败或宿主
 * 未上报）一律返回 null——信息不足时不做判断，避免把合法值误标为可疑。
 * 「不指定」（isUnspecifiedSetting：缺失 / 空串 / 'default'）不参与合法性判断。
 *
 * @param {object|null} route      当前档路由（{provider, model, effort}）
 * @param {object|null|undefined} catalog provider → {models, efforts, modelEfforts, error}
 * @returns {string|null} 注记文本；无可疑项或信息不足时 null
 */
export function acpConfigNote(route, catalog) {
  if (!route || !catalog || typeof catalog !== 'object') return null
  const entry = catalog[route.provider]
  ...
  const effort = clean(route.effort)
  const notes = []
  if (!isUnspecifiedSetting(model)) {
    const models = strList(entry.models)
    if (models.length > 0 && !models.includes(model)) {
      notes.push(`model=${model} 不在 ${route.provider} 模型目录中，产品侧可能回退默认模型`)
    }
  }
  if (!isUnspecifiedSetting(effort)) {
    const byModel = !isUnspecifiedSetting(model) && entry.modelEfforts && typeof entry.modelEfforts === 'object'
      ? strList(entry.modelEfforts[model])
      : []
    const efforts = byModel.length > 0 ? byModel : strList(entry.efforts)
    if (efforts.length > 0 && !efforts.includes(effort)) {
      notes.push(`effort=${effort} 不在 ${route.provider} 档位目录中，产品侧可能回退默认档位`)
    }
  }
  return notes.length > 0 ? notes.join('；') : null
}

/** 产品自报配置项的 id 约定（与 product-subagents 的 MODEL/EFFORT_CONFIG_IDS 对齐）。 */
const CONFIG_EFFORT_IDS = ['effort', 'reasoning_effort', 'thought_level']
/**
 * v1.11.12：从 P 侧 `config-option-error` 事件带的 configOptions 快照里取"产品实际
 * 生效值"（`currentValue`）。定位规则与 P 同构：**id 精确匹配优先 → category 兜底，
 * 且排除已被另一类认领的 id**（qoder 的 reasoning_effort 与 model 共享 category=model，
 * 只按 category 找会把 effort 当成 model）。找不到返回 null（不猜）。
 *
 * @param {Array<{id?:string,category?:string,currentValue?:string}>|null|undefined} configOptions
 * @param {'model'|'effort'} kind
 * @returns {string|null}
 */
export function effectiveConfigValue(configOptions, kind) {
  const list = Array.isArray(configOptions) ? configOptions.filter((o) => o && typeof o === 'object') : []
  if (!list.length) return null
  const effort = kind === 'effort'
  const ids = effort ? CONFIG_EFFORT_IDS : ['model']
  const exclude = effort ? ['model'] : CONFIG_EFFORT_IDS
  let hit = list.find((o) => ids.includes(o.id))
  if (!hit) hit = list.find((o) => o.category === (effort ? 'thought_level' : 'model') && !exclude.includes(o.id))
  const cur = hit && hit.currentValue
  return typeof cur === 'string' && cur.trim() ? cur.trim() : null
}

/**
 * v1.11.14(A)：待授权标记的 REST 序列化。
 * entry.permissionPending 现在是「每请求一条」的数组；旧客户端只读
 * `permissionPending`（单对象），故保留该字段 = 最早未决那条，新增
 * `permissionPendingList` = 全量。传入旧形态（单对象/null）也能出正确结果，
 * 便于滚动升级与测试。
 * @param {Array<object>|object|null} value
 * @returns {{permissionPending: object|null, permissionPendingList: object[]}}
 */
export function serializePermissionPending(value) {
  const rows = Array.isArray(value) ? value : (value && typeof value === 'object' ? [value] : [])
  const list = rows.map((r) => ({
    permId: r.permId ?? null,
    product: r.product ?? null,
    description: r.description ?? '未知操作',
    category: r.category ?? null,
    paths: Array.isArray(r.paths) ? r.paths : [],
    at: r.at ?? Date.now(),
  }))
  return { permissionPending: list[0] ?? null, permissionPendingList: list }
}
...
export class Dispatcher {
   ...
   * @param {() => object} [opts.productCatalog] v1.11.12：ACP provider 目录读取器（index.js 注入
   *        只读缓存），仅用于派发日志的「填了可能没生效」预检；缺省时该预检静默跳过。
   */
  constructor({ ctx, registry, dataDir, idleReleaseMs = DEFAULT_IDLE_RELEASE_MS, productCatalog = null }) {
    this.ctx = ctx
    this.registry = registry
    this.dataDir = dataDir
    this.productCatalog = typeof productCatalog === 'function' ? productCatalog : null
    ...
    /**
     * v1.9.3：外部子代理 ACP 权限挂起表 childId → 挂起请求数组。
     * v1.11.14(A)：值由「单条记录」改为「每请求一条的数组」——旧结构在同一
     * child 并发 2 条权限请求时后到者覆盖前者，用户点一次只消掉一条，剩下
     * 那条会从宿主弹窗通道（授权球蓝球）再次冒出来。
     * product-subagents 对所有 ACP 子代理（含非本插件派遣的宿主 product 子
     * 代理——product_delegate 等）都会发 'product-subagents/permission-pending'；
     * activeChildren 只覆盖本插件派遣的 agent 子代理，其余一律在此登记，供
     * /agent-api/active 合成条目透出 → 主窗口授权球可显示并决策（决策端点按
     * childId + permId 转发，无归属限制）。
     * 清理：permission-resolved 事件（摘对应那条）或 subagent/end（onChildEnd 整组删）。
     */
    this.externalPending = new Map()
    /**
     * v1.11.0：同任务去重注册表 dedupKey → Set<childId>。
     * 自动重试（failover）时检查同 key 是否有活跃兄弟，有则跳过重试。
     * 注册：dispatch 成功注册 activeChildren 时登记；
     * 注销：onChildEnd / closeChild 中清理。
     */
   ...
   */
  async dispatch(parentAgent, agentId, task, { runInBackground = true, viaSquad = null, squadRunId = null, stepIndex = null, totalSteps = null, waitResult = false, dedicatedChild = false, reuse = 'auto', childId = null, waitTimeoutMs = 3600000, failoverFrom = null, failoverCount = 0, failoverError = null, failoverTried = null } = {}) {
    ...
    const preferredRoutes = healthyRoutes.length > 0 ? healthyRoutes : rawRoutes
    // v1.11.10：换档链去重——本次 failover 链上【已尝试过】的 provider 不再作为新档候选。
    // 必需原因：health 只对 ACP relay 失败做 hard 立即冷却；非 ACP 档（宿主 spawn / LLM 路由）
    // 失败按 failThreshold 累计、不进冷却，于是换档重派会从 routes[0] 重新选回【刚刚失败的那一档】，
    // 白烧换档次数却永远走不到下一档。去掉 acpMode 前置条件（本次同版修复）后必须同时补这一条。
    const triedProviders = failoverCount > 0
      ? [...new Set([...(failoverTried ?? []), ...(failoverFrom ? [failoverFrom] : [])])]
      : []
    const triedSet = new Set(triedProviders)
    const remainedRoutes = triedSet.size > 0 ? preferredRoutes.filter((r) => r && !triedSet.has(r.provider)) : preferredRoutes
    const routes = remainedRoutes.length > 0 ? remainedRoutes : preferredRoutes

    // P2 换档文案（v1.11.10 修复）：来自 #retryOnChildFailure 的重试——在任务文本里
    // 带上换档说明，让新档 child 与主代理都明白这是链内重试（relay child 只会如实
    // 转发任务文本；主代理看到的仍是同一任务的继续）。
    // 必须在 routes 解析之后拼装：文案要按【目标档】的真实形态生成——旧实现硬编码
    // "（ACP 产品）"，末档 spawn child 会因此误以为自己是中继而调用 product_submit，
    // 必然报 "no remote product session is bound to this agent (recovery failed)"。
    if (failoverFrom && failoverCount > 0) {
      const targetProvider = routes[0]?.provider ?? null
      taskText = buildFailoverTaskText(taskText, {
        failoverFrom,
        failoverError,
        fromAcp: !!this.ctx.subagents?.getProvider?.(failoverFrom),
        targetAcp: !!this.ctx.subagents?.getProvider?.(targetProvider),
      })
    }
    ...
    // v1.11.11 修复（TDZ 回归）：reuseKey 必须在【使用它的两个分支】之前声明。
    // 旧实现把 `const reuseKey` 写在下方 v1.5.0 复用段（函数体后部），而 v1.5.3 的
    // 定向续聊分支会在【池外线程】路径上用它拼池键 →
    //   ReferenceError: Cannot access 'reuseKey' before initialization
    // 更糟的是此时 sendMessage 其实已经投递成功，却被当成"续聊失败"上报，主代理
    // 只好新开子代理（现场表现：追加任务不再续聊，凭空多出一个子代理、任务重复执行）。
    ...
    if (!dedicatedChild && childId) {
      ...
      try {
        ...
        // v1.11.0：同任务去重注册
        this.#registerActiveTask(childId, agentId, task)
        this.#log({
          kind: 'dispatch',
          agentId,
          agentName: agent.name,
        ...
        // v1.11.11：投递【已成功】时，后置记账出错不得把成功报成失败——否则主代理
        // 会误判"续聊不可用"而新开子代理，同一任务在两个线程里重复执行。
        if (delivered) {
          console.error('[dsh-agent-dispatch] 定向续聊已投递成功，后置记账失败（按成功处理）:', err?.message ?? err)
          return { agentId, agentName: agent.name, childId, taskLabel, output: null, ok: true }
        }
        throw new Error(
    ...
    if (!dedicatedChild && reuseMode !== 'fresh') {
      ...
      if (pooled) {
        ...
        try {
          ...
          // v1.11.0：同任务去重注册
          this.#registerActiveTask(pooled.childId, agentId, task)
          this.#log({
            kind: 'dispatch',
            agentId,
            agentName: agent.name,
    ...
    for (const route of candidates) {
      ...
      const agentOptions = route && !acpMode
        ? {
            ...
            model: route.model,
            ...(isUnspecifiedSetting(route.effort) ? {} : { reasoningEffort: route.effort }),
          }
        : undefined
      ...
      // 取值非法时回退产品自己的默认并告警）。
      // v1.11.12 增量 3：「不指定」= isUnspecifiedSetting（缺失 / 空串 / 'default'），
      // 命中就**完全不进 settings**——产品侧因此一个 set_config_option 都不会发，
      // 用自身默认值，而不是收到一个字面叫 default 的模型。
      // v1.11.12：与 configNote/configError 一起提到 try 外——失败分支的日志行要读它们
      // （此前 productSettings 声明在 try 内、catch 里引用会 ReferenceError，把"换档重试"
      // 打成整轮异常）。
      const productSettings = (() => {
        if (!acpMode || !route) return undefined
        const s = {}
        if (!isUnspecifiedSetting(route.model)) s.model = route.model
        if (!isUnspecifiedSetting(route.effort)) s.reasoningEffort = route.effort
        return Object.keys(s).length > 0 ? s : undefined
      })()
      let configError = null // 设置事件下发失败（不阻断任务）
      const configNote = acpMode ? this.#acpConfigNote(route) : null // 可疑 model/effort 注记
      ...
      try {
        ...
        if (acpMode && productSettings) {
          try {
            ...
            // v1.11.12：设置未送达 ≠ 任务失败。子代理仍在产品默认模型/档位下正常执行，
            // 因此不再记 ok:false（旧行为会让一次设置降级看起来像整轮失败，并触发
            // 不必要的自动换档判断），改记到本轮成功行的 configError。
            configError = `apply-child-settings failed: ${error && error.message ? error.message : error}`
          }
        }
        ...
        this.activeChildren.set(childId, {
          ...
          failoverTried: triedProviders, // v1.11.10：换档链上已尝试过的 provider（下一跳去重用）
          startedAt: Date.now(),
        ...
        this.#log({
          ...
          effort: acpMode ? (productSettings?.reasoningEffort ?? null) : (agentOptions?.reasoningEffort ?? null), // v1.11.12：补齐请求档位（null = 未指定，用默认）
          ...(configNote ? { configNote } : {}), // v1.11.12：可疑 model/effort 注记
          ...(configError ? { configError } : {}), // v1.11.12：设置下发失败（不阻断任务）
          reused: false,
        ...
        this.#log({
          ...
          provider: acpMode ? route.provider : (agentOptions?.provider ?? null), // v1.4.0：ACP 模式记录 route.provider
          model: acpMode ? (productSettings?.model ?? null) : (agentOptions?.model ?? null), // v1.6.1：ACP 记录 route 指定的产品模型
          effort: acpMode ? (productSettings?.reasoningEffort ?? null) : (agentOptions?.reasoningEffort ?? null), // v1.11.12
          ...(configNote ? { configNote } : {}), // v1.11.12：失败档若同时是可疑值，日志一眼可辨
          reused: false,
          taskLabel,
          taskText: taskText.slice(0, 4000), // v0.9.13：历史页任务详情（截断防膨胀）
          viaSquad, // v0.9.14：小队触发的委派带小队 id（历史页「类型」列）
    ...
    // v1.11.12：ACP 路由允许 model 为空（= 产品默认模型），此处不拼出 "provider/undefined"
    const tried =
      attempts.length > 0
        ? attempts.map((a) => [a.provider, a.model].filter(Boolean).join('/')).join(', ')
        : '（无 routes，继承父会话）'
    throw new Error(
  ...
  markChildSubmitFailed(info) {
    ...
    if (info.remoteSessionId) entry.remoteSessionId = info.remoteSessionId
  }
  ...
  /**
   * v1.11.12：product-subagents 的 `config-option-error` 落地——route 里写的
   * model/effort 被产品拒绝时，把「请求值 → 实际生效值」记一行观测日志。
   *
   * 纯只读观测：不改档、不重试、不标记失败（产品已按自己的默认值继续跑，任务本身
   * 是好的）。行 kind='config'，历史页聚合时被过滤（它不是一次委派），仅供排障 grep。
   * 只记标量：childId / provider / kind / requested / effective / error（截断），
   * 不落 configOptions 全量快照，也不碰任务正文。
   *
   * @param {{childId?:string|null, product?:string|null, kind?:string, requested?:any,
   *          error?:any, configOptions?:Array}} info
   */
  logConfigOptionError(info) {
    if (!info || typeof info !== 'object') return
    const kind = info.kind === 'model' || info.kind === 'effort' ? info.kind : null
    if (!kind) return // 未知 kind：不猜语义，直接丢弃（宁缺勿错）
    const childId = typeof info.childId === 'string' && info.childId ? info.childId : null
    const entry = childId ? this.activeChildren.get(childId) : null
    const scalar = (v) => (typeof v === 'string' ? v.trim().replace(/\s+/g, ' ').slice(0, 300) : typeof v === 'number' || typeof v === 'boolean' ? String(v) : null)
    const requested = scalar(info.requested)
    const effective = effectiveConfigValue(info.configOptions, kind)
    this.#log({
      kind: 'config',
      phase: 'option-error',
      childId,
      agentId: entry?.agentId ?? null,
      provider: entry?.provider ?? (typeof info.product === 'string' ? info.product : null),
      ...
      requested,
      effective,
      unchanged: !!(requested && effective && requested === effective),
      error: scalar(info.error),
      ok: true,
    })
  }

  /**
   * v1.11.14(A)：取某 child 的挂起权限请求数组（就地维护，保证返回引用可修改）。
   * @returns {Array<{permId: string, product: string|null, description: string, at: number}>}
   */
  #permissionSlot(entry) {
    if (!Array.isArray(entry.permissionPending)) entry.permissionPending = []
    return entry.permissionPending
  }

  /**
   * v1.11.14(A)：老 payload 无 permId 时的降级键——仍然每请求一条，绝不折叠。
   * 决议侧（`#hitPermId`）在 **permId 缺省**时按 FIFO 摘最早一条，与本处自增登记
   * 顺序天然对齐；permId 给了却没命中则忽略该决策（不顶替别的请求）。
   */
  #permIdOf(info) {
    if (typeof info.permId === 'string' && info.permId.trim()) return info.permId.trim()
    this._permSeq = (this._permSeq || 0) + 1
    return `legacy-${this._permSeq}`
  }
   ...
   * v1.11.14(A)：改为**每请求一条**（数组 + permId），并发授权不再互相覆盖。
   * @param {{childId: string, permId?: string, product?: string, description?: string,
   *          paths?: string[], category?: string|null, at?: number}} info
   */
  markPermissionPending(info) {
    if (!info || !info.childId) return
    const permId = this.#permIdOf(info)
    const record = {
      ...
      category: typeof info.category === 'string' && info.category ? info.category : null,
      cwd: info.cwd ?? null,
      parentSessionId: info.parentSessionId ?? null,
      remoteSessionId: info.remoteSessionId ?? null,
      at: info.at ?? Date.now(),
    }
    const entry = this.activeChildren.get(info.childId)
    if (!entry) {
      // v1.9.3：外部子代理（product_delegate 等宿主 product 子代理）——审批挂起
      // 仍须进授权球：登记 externalPending（不 inject 主代理消息，宿主 approval
      // 通道已有提示），由 /agent-api/active 合成条目透出。
      const list = this.externalPending.get(info.childId)
      const next = Array.isArray(list) ? list : []
      // 同 permId 重发（产品侧重试同一 toolCall）→ 就地更新，不堆重复行
      const dupAt = next.findIndex((r) => r.permId === permId)
      if (dupAt >= 0) next[dupAt] = { ...next[dupAt], ...record }
      else next.push(record)
      this.externalPending.set(info.childId, next)
      return
    }
    if (info.remoteSessionId) entry.remoteSessionId = info.remoteSessionId
    const slot = this.#permissionSlot(entry)
    const dupAt = slot.findIndex((r) => r.permId === permId)
    if (dupAt >= 0) slot[dupAt] = { ...slot[dupAt], ...record }
    else slot.push(record)
    const agent = this.registry.get(entry.agentId)
    this.#notifyParent(entry, `⏳ 子代理「${agent?.name ?? entry.agentId}」正在请求 ACP 权限：${record.product ? `[${record.product}] ` : ''}${record.description}。审批弹窗在该子代理的会话界面（运行中列表可见），请提醒用户前往点击允许/拒绝；未决前任务保持暂停。`)
  }

  /**
   * v1.11.14(M3)：决议命中判定——与插件侧 `pendingRegistry.take`（product-subagents
   * lib/permission-state.js）**同一套语义**，两处不得有第二种规则：
   *   · permId 给了却没命中 → null（忽略这条决策，绝不顶替别的请求）；
   *   · permId 缺省（老插件不发）→ 摘最早一条（FIFO，与登记顺序天然对齐）。
   */
  #hitPermId(rows, permId) {
    const wanted = typeof permId === 'string' && permId.trim() ? permId.trim() : ''
    if (!wanted) return rows[0] ? rows[0].permId ?? null : null
    return rows.some((r) => r && r.permId === wanted) ? wanted : null
  }

  /** 摘除 activeChildren entry 上对应的一条，返回被摘掉的记录（无则 null） */
  #dropEntryPending(entry, permId) {
    const slot = this.#permissionSlot(entry)
    if (slot.length === 0) {
      entry.permissionPending = null // #permissionSlot 可能刚把 null 规范化成 []，还原
      return null
    }
    const hitId = this.#hitPermId(slot, permId)
    if (hitId === null) return null
    const was = slot.find((r) => r.permId === hitId) || null
    const rest = slot.filter((r) => r.permId !== hitId)
    entry.permissionPending = rest.length === 0 ? null : rest // 全部决议才清标记
    return was
  }

  /** 摘除 externalPending（非本插件派遣的子代理）上对应的一条 */
  #dropExternalPending(childId, permId) {
    const stored = this.externalPending.get(childId)
    if (!stored) return null
    const rows = Array.isArray(stored) ? stored : [stored] // 兼容旧结构（单对象）
    if (rows.length === 0) {
      this.externalPending.delete(childId)
      return null
    }
    const hitId = this.#hitPermId(rows, permId)
    if (hitId === null) return null
    const was = rows.find((r) => r.permId === hitId) || null
    const rest = rows.filter((r) => r.permId !== hitId)
    if (rest.length === 0) this.externalPending.delete(childId)
    else this.externalPending.set(childId, rest)
    return was
  }
  ...
  markPermissionResolved(info) {
    if (!info || !info.childId) return
    const entry = this.activeChildren.get(info.childId)
    // v1.11.14(M4)：两张表**都**要清。登记挂起时是按"当时有没有 entry"分流的
    //（无 entry → externalPending），而 onChildEnd 会先删 entry 再等决议落定；
    // 于是晚到的 resolved 会走错分支，把另一张表里的记录永久留下——授权球上一
    // 行永不消失（该 childId 已不在 active 列表，前端也无从再清它）。
    const external = this.#dropExternalPending(info.childId, info.permId)
    const local = entry ? this.#dropEntryPending(entry, info.permId) : null
    const was = local || external
    if (!was) {
      if (info.permId && (entry || this.externalPending.has(info.childId))) {
        console.warn(`[dsh-agent-dispatch] permission-resolved permId=${info.permId} 未命中该 child 的挂起请求，忽略（child=${String(info.childId).slice(0, 8)}）`)
      }
      return
    }
    if (entry) {
      const agent = this.registry.get(entry.agentId)
      // v1.9.0：成功结局含 granted-session（本会话总是允许）/ granted-always（项目落盘）/
      // granted-once-fallback（落盘失败但本次放行）/ allowed-once（弹窗允许一次）
      const outcome = info && info.outcome ? String(info.outcome) : 'unknown'
      const ok = outcome === 'allowed-once' || outcome === 'granted-session' || outcome === 'granted-always' || outcome === 'granted-once-fallback'
      const label = outcome === 'granted-session' ? '已获批准（本会话总是允许）' : outcome === 'granted-always' ? '已获批准（总是允许，已落盘项目白名单）' : outcome === 'granted-once-fallback' ? '已放行本次（落盘失败未记忆）' : outcome === 'allowed-once' ? '已获批准' : `已结束(${outcome})`
      // v1.11.5：人为拒绝（rejected/deny）→ 标记本轮任务被用户否决——后续即使产品侧
      // 出现空正文/错误结束，也【不得自动换档重试】（换档等于绕过人的拒绝再问一次）。
      if (outcome === 'rejected' || outcome === 'deny') {
        entry._userRejectedAt = Date.now()
        // v1.11.6：拒绝后自动续聊引导——告知子代理该权限被拒、勿重试该路径，
        // 请改按其他方式继续推进任务（确无替代则如实上报终止）。
        // 优先解析父级 agent（agents 服务按 session id 索引，与产品侧用法一致）。
        try {
          const parentAgent = this.ctx.get?.('agents')?.get?.(entry.parentSessionId) ?? null
          if (parentAgent) {
            // v1.11.9：强指令——下一条 product_submit 的 task 只允许是纯指引原文本身，
            // 禁止附加标题/解释/重述/格式包装（产品会话保留完整上下文，续聊只需增量）。
            const pureGuide =
              `⛔ 用户拒绝了一项 ACP 权限请求：${was.description}。` +
              `该拒绝仅本次任务轮内有效——新一轮任务/重新派发后可重新请求该权限。` +
              `本任务轮内请勿重试该路径/操作（会被自动拦截）。` +
              `若任务本身已定义替代/降级方案，优先按任务定义推进；` +
              `否则可自行调整方案继续；仅当确无任何替代方案时，结束任务并如实上报"因权限被拒绝无法完成"。`
            const guide =
              `【强指令】下一条 product_submit 的 task 只允许是以下内容本身，` +
              `禁止附加任何标题、解释、重述或格式包装（产品会话保留完整上下文，这是续聊增量）：\n` +
              pureGuide
            this.followup(parentAgent, entry.childId, guide).catch((err) => {
              console.warn(`[dsh-agent-dispatch] 拒绝后引导续聊失败: ${err?.message ?? err}`)
            })
          }
        } catch (guideErr) {
          console.warn(`[dsh-agent-dispatch] 拒绝后引导续聊异常: ${guideErr?.message ?? guideErr}`)
        }
      }
      this.#notifyParent(
        entry,
        `${ok ? '✅' : '⛔'} 子代理「${agent?.name ?? entry.agentId}」的 ACP 权限请求${label}：${was.description}` +
        (ok ? '，任务继续执行。'
          : outcome === 'rejected' || outcome === 'deny'
            ? '，任务已被拒绝（本轮不会自动换档重试；如需重试请重新派发）。'
            : '，任务将按产品失败处理（自动换档或上报）。'),
      )
    }
  }
  ...
  /**
   * v1.11.1：订阅 product-subagents 的 'permission-unknown-session' 事件——
   * ACP 权限请求找不到 binding（binding/sessionId 失配，如产品守护进程重启窗口）
   * 时，产品侧已做富诊断 console.error + 事件广播；本层补：
   *   1) 本层富日志（带 deny 风暴 escalated 标记）；
   *   2) 尽力归属：匹配活跃子代理中 provider === product 的条目，向其父级会话
   *      注入可见提示（60s 去重；escalated 时每次风暴都提示）。
   * 未知会话无 childId 可路由 UI，故只能按 product 尽力归属；匹配不到仅落日志。
   */
  markUnknownPermission(info) {
    if (!info || typeof info !== 'object') return
    const { product, sessionId, description, paths, escalated, denyCount } = info
    const sessTail = typeof sessionId === 'string' ? sessionId.slice(-8) : String(sessionId ?? '?')
    const desc = description || '未知操作'
    const pathTxt = Array.isArray(paths) && paths.length ? paths.slice(0, 3).join(', ') : ''
    console.error(
      `[dsh-agent-dispatch] ACP 未知会话权限异常 product=${product} session=…${sessTail}` +
      (escalated ? `（deny 风暴 ${denyCount ?? '?'} 次）` : '') +
      ` ${desc}${pathTxt ? ' | ' + pathTxt : ''}`,
    )
    const now = Date.now()
    const dedup = this._unknownNotifyAt ?? (this._unknownNotifyAt = new Map())
    const targets = new Map() // parentSessionId → entry（同父只取一条）
    for (const [, entry] of this.activeChildren.entries()) {
      if (entry && entry.provider === product && entry.parentSessionId && !targets.has(entry.parentSessionId)) {
        targets.set(entry.parentSessionId, entry)
      }
    }
    // 自动关闭（v1.11.2）：deny 风暴（escalated）且该 product 的活跃候选**唯一**时，
    // 失配会话几乎可以确定属于该子代理——直接关闭它（emit child-closed →
    // product-subagents 立即 dispose 远程、删 binding、不再复活），并先通知父级。
    if (escalated && targets.size === 1) {
      const [pid, entry] = [...targets.entries()][0]
      const childId = entry.childId
      const agent = this.registry.get(entry.agentId)
      this.#notifyParent(
        entry,
        `⚠️ ACP 权限异常（deny 风暴 ${denyCount ?? '?'} 次）：${product} 会话 …${sessTail} 的权限请求找不到绑定` +
        `——已自动关闭子代理「${agent?.name ?? entry.agentId}」（${String(childId).slice(0, 8)}…）。如需重试请重新派发。`,
      )
      try { this.#unregisterActiveTask(childId, entry.agentId, entry.task) } catch { /* ignore */ }
      this.activeChildren.delete(childId)
      if (this.byAgent.get(entry.agentId) === childId) this.byAgent.delete(entry.agentId)
      try { this.ctx.emit('product-subagents/child-closed', { childId, immediate: true }) } catch { /* ignore */ }
      this.#log({
        kind: 'close',
        ...
        parentSessionId: pid,
        closing: 'running',
        auto: 'unknown-session',
        reason: 'deny-storm',
        ok: true,
      })
      console.warn(`[dsh-agent-dispatch] 已自动关闭未知会话失配的子代理 ${String(childId).slice(0, 8)}…（${product} deny 风暴 ${denyCount ?? '?'} 次）`)
      return
    }
    // 未升级（首次/偶发，可能只是重连瞬态）：仅提示不关闭；升级但多候选无法判定归属：
    // 也只提示不关闭（60s 去重防刷屏）。
    const candidateTxt = [...targets.values()]
      .map((e) => `${e.agentId ?? '?'}(${String(e.childId).slice(0, 8)}…${e.remoteSessionId ? ' 远程…' + String(e.remoteSessionId).slice(-8) : ''})`)
      .join('、')
    for (const [parentSessionId, entry] of targets.entries()) {
      const key = `${product}|${sessTail}|${parentSessionId}`
      const lastAt = dedup.get(key) ?? 0
      if (now - lastAt < 60000) continue
      dedup.set(key, now)
      this.#notifyParent(
        entry,
        `⚠️ ACP 权限异常：${product} 会话 …${sessTail} 的权限请求找不到绑定` +
        (escalated ? `（deny 风暴 ${denyCount ?? '?'} 次，同 product 有多个活跃子代理无法判定归属，未自动关闭）` : '（疑似 binding 失配或重连瞬态，暂不自动关闭）') +
        `：${desc}${pathTxt ? ' | ' + pathTxt : ''}。` +
        (escalated ? `同 product 活跃子代理：${candidateTxt}——请对照远程会话尾号关闭失配的那个（会话 …${sessTail} 不在其 binding 中）后重新派发。` : `若对应子代理持续卡住（无授权球/无响应），请手动关闭后重新派发。`),
      )
    }
  }
   ...
   * + content + source）。
   *
   * v1.11.13：宿主 session log 升到 **format v4** 后，`source.kind === 'plugin'`
   * 这种 V3 插件包装被 v4 硬拒（`@deepseek-ai/dsh-session-format-v3-to-v4` 的
   * `assertV4SourceRowAdmission` → "format v4 message requires a producer-owned
   * source kind"）。**inject 是同步 void、当场不报错**：坏 source 要等到下一轮把
   * inbox 消息 splice 进会话、按行落盘校验时才炸，于是表现为「ACP 子代理请求授权
   * 之后，主代理继续聊天本轮必失败」——通知是装饰性的，却打死了整轮。v4 给第三方
   * 生产者的标识是 `plugin:<包名>`（与 v3→v4 迁移改写
   * `{kind:'plugin',plugin:'<包名>'}` 的结果逐字一致），故改为下面这一行。宿主
   * 同形态的先例见 `dsh-subagent` 的 createSettlementMessage（`kind:'subagent-settled'`）。
   */
  #notifyParent(entry, text) {
    try {
      ...
      parent.inject({
        ...
        source: { kind: 'plugin:dsh-agent-dispatch' },
      })
    } catch (err) {
  ...
  onChildEnd(childId, stopReason, lastAssistantMessage) {
    if (!childId) return
    // v1.9.3：外部子代理终结 → 清其 ACP 挂起登记（未决议即结束，不得残留授权球）
    this.externalPending.delete(childId)
    ...
    if (this.byAgent.get(entry.agentId) === childId) this.byAgent.delete(entry.agentId)
    // v1.11.0：同任务去重注销
    this.#unregisterActiveTask(childId, entry.agentId, entry.task)
    const agent = this.registry.get(entry.agentId)
    ...
    // v1.11.5：人为拒绝（_userRejectedAt）的任务即使以产品故障结束也【不自动换档】——
    // 拒绝是人的决定，换档重试等于绕过拒绝；只有真正的系统/产品错误才走 fallback 链。
    // v1.11.10：去掉 entry.acpMode 前置条件——非 ACP 档（宿主 spawn / LLM 路由）失败同样
    // 应沿 routes 链换档。旧实现只允许 ACP relay 档换档，导致 routes 首档是模型路由的
    // Agent 完全拿不到 fallback；且末档失败后链已走完，行为由 #canFailover 的档位/次数
    // 判定天然兜底（不会原地自旋，见 1282/1284）。
    const willFailover = !ok && !entry._userRejectedAt && (reason === 'error' || productFailed) && !!entry.provider && entry.task && this.#canFailover(entry)
    if (w && !willFailover) {
      clearTimeout(w.timer)
      this.waiters.delete(childId)
      // v1.11.10：失败不再静默——本轮本具备换档资格却被卡住时（后续档位全冷却 / 已是最末档 /
      // 换档次数耗尽 / 同任务活跃兄弟去重），把具体原因一并回传，避免上层看到"链走完了"
      // 却无从判断是产品故障、还是调度策略主动停止。
      const blockHint = !ok && !entry._userRejectedAt && (reason === 'error' || productFailed)
        ? this.#failoverBlockReason(entry)
        : ''
      w.resolve(ok ? norm : `（步骤失败: ${reason}${productFailed ? `; 产品故障: ${entry._submitFailed?.code ?? ''} ${entry._submitFailed?.message ?? ''}`.trim() : ''}${blockHint}）`)
    }
   ...
   * v1.11.10：不再要求 entry.acpMode——非 ACP 档（宿主 spawn / LLM 路由）同样参与换档；
   * 末档与次数上限由下方 idx/failoverCount 判定兜底。
   * @param {object} entry activeChildren 条目（含 provider/acpMode/failoverCount/task）
   */
  #canFailover(entry) {
    if (!entry || !entry.provider || !entry.task) return false
    // v1.11.0：同任务去重——有活跃兄弟在跑则跳过自动重试
    const key = dedupKey(entry.agentId, entry.task)
    const siblings = this.activeTasks.get(key)
    if (siblings && siblings.size > 0) {
      // 排除自身（onChildEnd 尚未注销）
      const others = [...siblings].filter((cid) => cid !== entry.childId && this.activeChildren.has(cid))
  ...
  /**
   * v1.11.10：失败后【未自动换档】的原因诊断——只用于失败信息回传，不改变换档决策
   * （决策仍由 #canFailover 独家负责；本方法不得被用作 gate）。
   * @param {object} entry activeChildren 条目
   * @returns {string} '' 表示无需说明；否则返回以"；自动换档已停止: "开头的说明
   */
  #failoverBlockReason(entry) {
    if (!entry || !entry.provider) return ''
    const routes = this.registry.resolveRoutes(entry.agentId)
    if (!routes || routes.length < 2) return '' // 单档/无档：本就没有 fallback 链，无需提示
    const idx = routes.findIndex((r) => r && r.provider === entry.provider)
    if (idx < 0) return ''
    if (idx >= routes.length - 1) return '；自动换档已停止: 当前已是最末档，无后续路由'
    if ((entry.failoverCount ?? 0) >= routes.length - 1) return '；自动换档已停止: 本次任务换档次数已达上限'
    const nextReady = routes.slice(idx + 1).some((r) => r && !this.health.isCooling(entry.agentId, r.provider))
    if (!nextReady) return '；自动换档已停止: 后续档位全部处于冷却中（60s×3ⁿ，上限 600s），需等待冷却或人工改派'
    return '；自动换档已停止: 同任务已有活跃兄弟线程（去重跳过）'
  }

  #registerActiveTask(childId, agentId, task) {
    const key = dedupKey(agentId, task)
    let set = this.activeTasks.get(key)
    if (!set) { set = new Set(); this.activeTasks.set(key, set) }
    set.add(childId)
  }

  #unregisterActiveTask(childId, agentId, task) {
    if (!task) return
    const key = dedupKey(agentId, task)
    const set = this.activeTasks.get(key)
    if (!set) return
    set.delete(childId)
    if (set.size === 0) this.activeTasks.delete(key)
  }
  ...
  async #retryOnChildFailure(childId, entry, reason, norm) {
    ...
    const { childId: nextChildId } = await this.dispatch(parent, entry.agentId, entry.task, {
      ...
      failoverTried: entry.failoverTried ?? null, // v1.11.10：换档链去重（防重选已失败档位）
    })
    this.#log({
      kind: 'dispatch',
      agentId: entry.agentId,
  ...
  async closeChild(parentAgent, { childId, agentId } = {}) {
    ...
    for (const t of targets) {
      ...
      // v1.11.0：同任务去重注销
      const activeEntry = this.activeChildren.get(t.childId)
      if (activeEntry) this.#unregisterActiveTask(t.childId, t.agentId, activeEntry.task)
      const running = this.activeChildren.has(t.childId)
      if (!running) await this.#drainChild(t.parentSessionId, t.childId)
      // v1.11.0(A)：close → emit product-subagents/child-closed 通知远程立即 dispose
      try {
        this.ctx.emit('product-subagents/child-closed', { childId: t.childId, immediate: true })
      } catch { /* 事件发射失败不影响 close 主流程 */ }
  ...
  purgeParent(parentSessionId) {
    ...
    // v1.11.0：清理该父会话的 activeTasks 去重条目
    for (const [key, set] of [...this.activeTasks.entries()]) {
      for (const cid of [...set]) {
        const entry = this.activeChildren.get(cid)
        if (entry && entry.parentSessionId === parentSessionId) {
          set.delete(cid)
        }
      }
      if (set.size === 0) this.activeTasks.delete(key)
  ...
  dispose() {
    ...
    this.poolBusy.clear()
    this.activeTasks.clear() // v1.11.0
  }
  ...
  /**
   * v1.11.12：ACP provider 目录预检（见 acpConfigNote）。目录读取失败一律降级为
   * 无注记——可观测性是加分项，绝不能反过来影响派发。
   */
  #acpConfigNote(route) {
    if (!this.productCatalog || !route) return null
    try {
      return acpConfigNote(route, this.productCatalog())
    } catch {
      return null
    }
  }
