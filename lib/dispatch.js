...
export class Dispatcher {
  ...
  constructor({ ctx, registry, dataDir, idleReleaseMs = DEFAULT_IDLE_RELEASE_MS, productCatalog = null }) {
    ...
    /**
     * v1.9.3：外部子代理 ACP 权限挂起表 childId → { childId, product, description,
     * paths, cwd, parentSessionId, at }。product-subagents 对所有 ACP 子代理（含
     * 非本插件派遣的宿主 product 子代理——product_delegate 等）都会发
     * 'product-subagents/permission-pending'；activeChildren 只覆盖本插件派遣的
     * agent 子代理，其余一律在此登记，供 /agent-api/active 合成条目透出 →
     * 主窗口授权球可显示并决策（决策端点按 childId 转发，无归属限制）。
     * 清理：permission-resolved 事件或 subagent/end（onChildEnd）删除。
     */
  ...
  markPermissionPending(info) {
    ...
    const entry = this.activeChildren.get(info.childId)
    if (!entry) {
      // v1.9.3：非本插件派遣的子代理（product_delegate 等宿主 product 子代理）——
      // 审批挂起仍须进授权球：登记 externalPending（不 inject 主代理消息，宿主
      // approval 通道已有提示），由 /agent-api/active 合成条目透出。
      this.externalPending.set(info.childId, {
  ...
  markPermissionResolved(info) {
    ...
    const entry = this.activeChildren.get(info.childId)
    if (!entry) {
      // v1.9.3：外部子代理（product_delegate 等）决议 → 清 externalPending
      this.externalPending.delete(info.childId)
      return
    }
