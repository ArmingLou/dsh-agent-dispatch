 ...
 *     错误并附已尝试的 provider/model 列表。同一Agent已有活跃 child 时复用
 *     旧 child 走 followup 续聊，不重复新建。
 *   - followup()：对已存在的Agent child 追问，上下文延续。
...
export class Dispatcher {
  ...
  constructor({ ctx, registry, dataDir }) {
    ...
    /** v0.9.40：agentId → childId 二级索引（同一Agent最新活跃 child，供复用续聊） */
   ...
   *   2. 同一Agent已有活跃 child → 复用旧 child 走 followup 续聊（续聊失败
  ...
  async dispatch(parentAgent, agentId, task, { runInBackground = true, viaSquad = null, squadRunId = null, stepIndex = null, totalSteps = null, waitResult = false, dedicatedChild = false, waitTimeoutMs = 3600000 } = {}) {
    ...
    // 与创建子代理时注入的 toolFilter.deny 双保险——即便 deny 被绕过，这里仍物理阻断
    // 第三层及更深层委派，防止低成本子代理把任务再派回付费模型（如 architect/deepseek-official）。
    ...
    // 同一Agent已有活跃 child：复用续聊，把任务作为消息发过去，不新建
    // v0.9.40：byAgent 二级索引（agentId → childId）替代旧的 agentId 主 key；
    // dedicatedChild=true（小队步骤）跳过复用，强制新建专属子代理
    const existingChildId = dedicatedChild ? undefined : this.byAgent.get(agentId)
    ...
    if (existingChildId && existingEntry) {
      try {
        ...
        // v0.9.40：以 childId 为主 key 回写（Agent索引同步指向该 child）
        ...
        this.byAgent.set(agentId, existingChildId)
        // v0.9.36：续聊同样默认等结果
        const output = waitResult
          ? await this.#waitFor(existingChildId, waitTimeoutMs)
          : null
        return { agentId, agentName: agent.name, childId: existingChildId, taskLabel, output, ok: true }
      } catch (err) {
        ...
        const { childId } = await this.ctx.subagents.startContinuable({
          ...
          request: {
            ...
            ...(acpMode
              ...
              : { toolFilter: { deny: [
                  'agent_dispatch', 'agent_followup', 'agent_list', 'agent_squad', 'agent_squad_continue', 'agent_squad_upsert', 'agent_import_skill', 'agent_upsert',
                  // v1.4.1 递归护栏扩展：堵住"通用子代理工具"漏洞——此前只 deny 本插件委派工具，
                  // 子代理仍可用宿主通用 subagent/subagent_fork/workflow/product_delegate 等另起第三层代理（可能路由回付费模型）。
                  'subagent', 'subagent_fork', 'subagent_progress', 'list_agents', 'interrupt_agent', 'send_message',
                  'product_delegate', 'product_submit', 'product_wait', 'product_roles',
                  'workflow', 'ralph',
                  'create_goal', 'get_goal', 'update_goal',
                ] } }), // v1.1.2 递归护栏：子代理看不到委派工具，物理阻断再派下级；v1.2.0 加 agent_upsert 防子代理改注册表；v1.3.0 加 agent_squad_continue 防子代理续跑小队；v1.3.1 加 agent_squad_upsert 防子代理改小队注册表；v1.4.0 ACP 模式改用 allow 白名单；v1.4.1 扩展通用工具 deny 名单
            maxDepth: Math.max(2, (parentAgent?.options?.subagentDepth ?? 0) + 1), // 委派深度记账；递归由 toolFilter.deny 阻断
          },
        })
        // v0.9.40：childId 为主 key（多子代理并存）；byAgent 二级索引供直接委派复用续聊
