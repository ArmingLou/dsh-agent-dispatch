...
export class Dispatcher {
   ...
   */
  async dispatch(parentAgent, agentId, task, { runInBackground = true, viaSquad = null, squadRunId = null, stepIndex = null, totalSteps = null, waitResult = false, dedicatedChild = false, reuse = 'auto', childId = null, waitTimeoutMs = 3600000 } = {}) {
    ...
    if (!dedicatedChild && childId) {
      ...
      try {
        ...
        if (pooled) {
          ...
          // 池外线程（如 list_agents 里看到的、或已被淘汰的）→ 补记一条池条目，
          // 使后续 auto 复用也能命中它
          const entry = {
  ...
  /**
   * v1.5.3：列出当前会话的子代理线程（agent_children 工具）——复用池条目 +
   * 活跃但不在池的 child（fresh 策略/小队专属），附最近任务标签与状态。
