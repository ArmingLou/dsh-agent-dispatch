        ...
        const { childId } = await this.ctx.subagents.startContinuable({
          ...
          request: {
            ...
            ...(acpMode
              ...
              : { toolFilter: { deny: ['agent_dispatch', 'agent_followup', 'agent_list', 'agent_squad', 'agent_squad_continue', 'agent_squad_upsert', 'agent_import_skill', 'agent_upsert'] } }), // v1.1.2 递归护栏：子代理看不到委派工具，物理阻断再派下级；v1.2.0 加 agent_upsert 防子代理改注册表；v1.3.0 加 agent_squad_continue 防子代理续跑小队；v1.3.1 加 agent_squad_upsert 防子代理改小队注册表；v1.4.0 ACP 模式改用 allow 白名单
