...
//
// 文案统一：面板与设置入口统称「Agent 调度」（工具名 expert_*
// 保持不变，那是 REST 契约）。与宿主半通过同源 REST（/expert-api*）
// 通信，保存即生效免重启。
//
// 硬约束备忘：
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS =
      ...
      ".ad-step-row .ad-input.expert{flex:none;width:190px}" +
    ...
    // 注意：/expert-api/active 与 dispatches 的 ts 均为 ISO 字符串，
    ...
    function squadStepsText(steps) {
      ...
      return layers
        .map((layer) => layer.map((i) => (list[i] && (list[i].phase || list[i].expertId)) || "步骤" + (i + 1)).join("｜"))
        .join(" → ");
    }

    // ── 专家编辑表单（新建 / 编辑共用；initial 为 null 表示新建；onDelete 非空时表单尾显示删除按钮）──
    function ExpertForm({ initial, isNew, onSave, onCancel, onDelete, models, defaultModel }) {
    ...
    // experts: [{ id, name }]（调度页 data.experts 提供；空数组时 expertId 退化为手动输入）
    // onSave 约定返回 Promise：服务端校验失败时在表单内显示错误条（失败保留表单）。
    function SquadForm({ initial, isNew, experts, onSave, onCancel, onDelete }) {
      const expertList = Array.isArray(experts) ? experts.filter((e) => e && e.id) : [];

      const [f, setF] = React.useState(() => {
        const blankStep = { expertId: "", phase: "", dependsOn: [], instruction: "" };
        if (initial) {
      ...
      const addStep = () => setF(Object.assign({}, f, { steps: f.steps.concat([{ expertId: "", phase: "", dependsOn: [], instruction: "" }]) }));
      ...
      // expertId 下拉选项：Agent 列表 name（id）；当前值不在列表（Agent 已删/内置组队引用未注册 Agent）时追加提示项防 select 空白
      const expertOptions = (cur) => {
        const opts = [React.createElement("option", { key: "__empty__", value: "" }, "选择 Agent…")];
        for (const ex of expertList) {
          opts.push(React.createElement("option", { key: ex.id, value: ex.id }, (ex.name || ex.id) + "（" + ex.id + "）"));
        }
        if (cur && !expertList.some((ex) => ex.id === cur)) {
      ...
      const save = () => {
        ...
        for (let i = 0; i < f.steps.length; i++) {
          const st = f.steps[i];
          if (!st.expertId.trim()) { setErr("步骤 " + (i + 1) + " 缺 expertId"); return; }
      ...
      const stepItems = f.steps.map((st, i) =>
        React.createElement("div", { key: i, className: "ad-step-item" },
          React.createElement("div", { className: "ad-step-row" },
            ...
            expertList.length
              ? React.createElement("select", { className: "ad-input ad-select expert", title: "该步骤委派给哪个 Agent", value: st.expertId, onChange: (e) => setStep(i, "expertId", e.target.value) },
                  expertOptions(st.expertId),
                )
              : React.createElement("input", { className: "ad-input expert", placeholder: "expertId（如 log-tracer）", value: st.expertId, onChange: (e) => setStep(i, "expertId", e.target.value) }),
            ...
            f.steps.length > 1
              ? f.steps.map((other, j) =>
                  j === i ? null : React.createElement("label", { key: j, className: "ad-dep-check", title: "勾选后该步骤等待步骤 " + (j + 1) + " 完成" },
                    ...
                    React.createElement("span", null, "S" + (j + 1) + " " + (other.phase || other.expertId || "?")),
                  ),
                )
        ...
        React.createElement("div", { className: "ad-field" },
          ...
          React.createElement("div", { className: "ad-flow-preview" },
            ...
            React.createElement("div", { className: "ad-flow-preview-fixed" },
              React.createElement(SquadFlowGraph, { steps: f.steps || [], experts: experts }),
            ),
          ),
        ),
        err ? React.createElement("div", { className: "ad-err" }, err) : null,
    ...
    function ManageTab() {
      const [data, setData] = React.useState(null);                  // GET /expert-api 结果
      const [editing, setEditing] = React.useState(null);            // null | { isNew: bool, expert: object|null }
      ...
      const toggle = (ex) =>
        apiPost("/expert-api/toggle", { id: ex.id, enabled: !ex.enabled }).then(refresh).catch((e) => setErr(msg(e)));
      const upsert = (expert) =>
        apiPost("/expert-api/upsert", { expert })
          .then(() => { setEditing(null); refresh(); })
          .catch((e) => setErr(msg(e))); // 失败保留表单，仅显示全局错误条
      const doRemove = () => {
        const ex = confirmDel;
        setConfirmDel(null);
        apiPost("/expert-api/remove", { id: ex.id }).then(refresh).catch((e) => setErr(msg(e)));
      };

      const experts = data ? (data.experts || []) : [];
      // v0.9.5：卡片点整体即编辑；编辑/删除移到 hover 浮层图标按钮（视觉降噪）
      const cards = experts.map((ex) => {
        ...
        return React.createElement("div", {
          ...
          onClick: () => setEditing({ isNew: false, expert: ex }),
        },
      ...
      return React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "14px" } },
        React.createElement("div", { className: "ad-kicker-row" },
          React.createElement("span", { className: "ad-kicker" }, "Agents · " + (experts.length || "…")),
          React.createElement("button", { className: "ad-btn mini primary", onClick: () => setEditing({ isNew: true, expert: null }) }, "+ 新建 Agent"),
        ),
        err ? React.createElement("div", { className: "ad-err" }, err) : null,
          ...
          ? React.createElement("div", { className: "ad-modal-mask", onClick: () => setEditing(null) },
              React.createElement("div", { className: "ad-modal form", onClick: (e) => e.stopPropagation() },
                ...
                React.createElement("div", { className: "ad-modal-body" },
                  React.createElement(ExpertForm, {
                    key: editing.isNew ? "__new__" : (editing.expert && editing.expert.id) || "__edit__",
                    initial: editing.expert,
                    isNew: editing.isNew,
                    onSave: upsert,
                    onCancel: () => setEditing(null),
                    // v0.9.7：弹窗内删除 → 关表单 → 弹删除确认
                    onDelete: editing.isNew ? null : () => { const ex = editing.expert; setEditing(null); setConfirmDel(ex); },
        ...
        delExpert
          ? React.createElement("div", { className: "ad-modal-mask", onClick: () => setConfirmDel(null) },
              React.createElement("div", { className: "ad-modal", style: { width: "380px" }, onClick: (e) => e.stopPropagation() },
                ...
                React.createElement("div", { className: "ad-modal-body" },
                  React.createElement("div", { className: "ad-modal-desc" }, "确定删除「" + (delExpert.name || delExpert.id) + "」？"),
                ),
      ...
      return React.createElement("div", {
        ...
        onCancel
          ? React.createElement("button", {
              ...
              disabled: cancelling === a.childId, // v0.9.40：按 childId 精确取消（同专家多卡并发）
    ...
    function OverviewTab() {
      const [ov, setOv] = React.useState(null); // GET /expert-api/overview 结果
      const [act, setAct] = React.useState(null); // GET /expert-api/active 结果
      const [meta, setMeta] = React.useState(null); // v0.9.36：GET /expert-api（默认模型/数据目录）结果
      const [err, setErr] = React.useState("");
      const [now, setNow] = React.useState(Date.now()); // 运行时长每 10s 走字
      const [cancelling, setCancelling] = React.useState(null); // 正在中止的 expertId
      const [fabOn, setFabOn] = React.useState(isFabVisible()); // v0.9.36：悬浮球总开关

      const cancel = (a) => {
        // v0.9.40：按 childId 精确取消（同专家多卡并发时，expertId 兜底会误伤）
        setCancelling(a.childId);
        apiPost("/expert-api/cancel", { childId: a.childId, expertId: a.expertId })
          .catch((e) => setErr(msg(e)))
          .finally(() => { setCancelling(null); apiGet("/expert-api/active").then(setAct).catch(() => {}); });
      };

      const refreshOv = () => apiGet("/expert-api/overview").then(setOv).catch((e) => setErr(msg(e)));
      const refreshAct = () => apiGet("/expert-api/active").then(setAct).catch(() => {});
      const refreshMeta = () => apiGet("/expert-api").then(setMeta).catch(() => {});
      React.useEffect(() => {
    ...
    function HistoryTab() {
      const [list, setList] = React.useState(null); // GET /expert-api/dispatches 结果
      ...
      const refresh = () => {
        apiGet("/expert-api/dispatches?limit=200")
          .then((d) => setList(d.dispatches || []))
          .catch((e) => setErr(msg(e)));
        apiGet("/expert-api/active")
      ...
      const agentLines = agentRows.map((d, i) => {
        ...
        const delBtn = React.createElement("button", {
          ...
          onClick: () => {
            ...
            apiPost("/expert-api/history/remove", { ts: d.ts }).then(refresh).catch((e) => setErr(msg(e)));
          },
        }, "删除");
        ...
        const preview = previewKey === key
          ? React.createElement("div", { className: "ad-hist-preview" },
              React.createElement("div", { className: "ad-hist-preview-row" },
                ...
                (d.emoji || "") + (d.expertName || d.expertId || "?") + " · " + (d.provider || "?") + "/" + (d.model || "?")),
        ...
        return React.createElement("div", { key, className: "ad-hist-line" + (previewKey === key ? " open" : "") },
          histHead({
            time: fmtTs(d.ts),
            avatar: React.createElement(Avatar, { name: d.expertName || d.expertId, emoji: d.emoji }),
            name: d.expertName || d.expertId || "?",
            nameTitle: (d.expertId || "") + " · " + (d.provider || "?") + "/" + (d.model || "?"),
            summary: d.taskLabel,
      ...
      const runLines = runs.map((run) => {
        ...
        const delRunBtn = React.createElement("button", {
          ...
          onClick: () => {
            ...
            if (run.noTopo) {
              Promise.all(run.disps.map((d) => apiPost("/expert-api/history/remove", { ts: d.ts }).catch(() => null))).then(refresh);
            } else {
              apiPost("/expert-api/history/remove-run", { squadRunId: run.id }).then(refresh).catch((e) => setErr(msg(e)));
            }
          },
        }, "删除");
        ...
        const topoSteps = (run.steps || []).map((s) => ({ phase: s.phase, expertId: s.expertId, dependsOn: s.dependsOn || [] }));
        const statuses = (() => {
          const arr = [];
        ...
        const stepRows = (() => {
          ...
          for (let i = 0; i < totalN; i++) {
            ...
            const nm = (meta.phase || "") + (meta.expertId && meta.phase !== meta.expertId ? " · " + (disp && disp.expertName ? disp.expertName : meta.expertId) : "");
    ...
    function SquadFlowGraph({ steps, experts, large, onOpen, statuses }) {
      let layers = [];
      ...
      const expertOf = (id) => (Array.isArray(experts) ? experts.find((e) => e.id === id) : undefined);
      const els = [];
      // 层标注：同层=并行（｜），跨层=串行（→）
      if (large) {
      ...
      steps.forEach((st, i) => {
        ...
        const label = st.phase || st.expertId;
        const stepNum = "S" + (i + 1);
    ...
    function SquadFlowModal({ squad, experts, onClose }) {
      ...
      const stepRows = steps.map((st, i) => {
        return React.createElement("div", { key: i, className: "ad-modal-step" },
          React.createElement("div", { className: "ad-modal-step-h" },
            ...
            React.createElement("span", { className: "ad-modal-step-name" }, st.phase || st.expertId),
        ...
        React.createElement("div", { className: "ad-modal", onClick: (e) => e.stopPropagation() },
          ...
          React.createElement("div", { className: "ad-modal-body" },
            ...
            React.createElement("div", { className: "ad-modal-graph" },
              React.createElement(SquadFlowGraph, { steps: steps, experts: experts, large: true }),
            ),
    ...
    function SquadTab() {
      const [data, setData] = React.useState(null);                  // GET /expert-api（拿 experts 供下拉）
      const [squads, setSquads] = React.useState(null);              // GET /expert-api/squads 结果
      ...
      const upsertSquad = (squad) =>
        apiPost("/expert-api/squad/upsert", { squad })
          .then(() => { setEditingSquad(null); refresh(); });
      const doRemoveSquad = () => {
        const sq = confirmDelSquad;
        setConfirmDelSquad(null);
        apiPost("/expert-api/squad/remove", { id: sq.id }).then(refresh).catch((e) => setErr(msg(e)));
      };

      const experts = data ? (data.experts || []) : [];
      const toggleSquad = (sq) =>
        apiPost("/expert-api/squad/toggle", { id: sq.id, enabled: !sq.enabled }).then(refresh).catch((e) => setErr(msg(e)));
        ...
        React.createElement("div", {
          ...
          React.createElement("div", { className: "ad-squad-card-main" },
            ...
            React.createElement("div", { className: "ad-graph-box", title: "点击放大查看执行流", onClick: (e) => { e.stopPropagation(); setViewSquad(sq); } },
              React.createElement(SquadFlowGraph, { steps: sq.steps || [], experts: experts }),
            ),
      ...
      return React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "14px" } },
              ...
              React.createElement("div", { className: "ad-modal form wide", onClick: (e) => e.stopPropagation() },
                ...
                React.createElement("div", { className: "ad-modal-body" },
                  React.createElement(SquadForm, {
                    ...
                    experts: experts, // v0.8.2：完整列表（含 emoji），供执行流预览图使用
                    onSave: upsertSquad,
        ...
        viewSquad
          ? React.createElement(SquadFlowModal, { squad: viewSquad, experts: experts, onClose: () => setViewSquad(null) })
          : null,
      );
    }
    ...
    function AgentPanel() {
      ...
      React.useEffect(() => {
        const refresh = () => apiGet("/expert-api/active").then((d) => setAct({ active: d.active || [] })).catch(() => {});
        refresh();
      ...
      const renderPop = (items) => {
        ...
        if (!items) {
          ...
          for (const a of items) {
            ...
            else setAvatarEl(em, a.emoji, a.expertName || a.expertId); // v0.9.14：首字头像
            const g = document.createElement("span");
            g.className = "grow";
            const t1 = document.createElement("div");
            t1.className = "t1";
            t1.textContent = isSquadRun ? squadName + " · 小队" : (a.expertName || a.expertId || "?");
        ...
        const mkRecentCard = (d) => {
          ...
          const em = document.createElement("span");
          setAvatarEl(em, d.emoji, d.expertName || d.expertId);
          const g = document.createElement("span");
          g.className = "grow";
          const t1 = document.createElement("div");
          t1.className = "t1";
          t1.textContent = d.expertName || d.expertId || "?";
          const t2 = document.createElement("div");
          t2.className = "t2";
          ...
          if (typeof ctx.effect === "function") {
            ctx.effect(() => inputTriggers.registerSource({
              ...
              candidates: async (session, req) => {
                ...
                try {
                  const d = await apiGet("/expert-api/suggest?q=" + encodeURIComponent(q));
                  if (!d.ok) return [];
                  const rows = [];
