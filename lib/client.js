  ...
  factory: (require) => {
    ...
    function mountPermFab() {
      ...
      const sendDecision = (a, answer, label) => {
        ...
        apiPost("/agent-api/permission-decision", { childId: a.childId, permId: a.permId || null, answer })
          .then(() => {
      ...
      const renderPop = () => {
        ...
        for (const a of lastPending) {
          ...
          // v1.11.14(D)：会话期授权按"类别"记忆时，告诉用户将被记住的是什么
          const catTip = a.permissionPending.category ? "（本会话将记住：" + a.permissionPending.category + "）" : "";
