...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS =
      ...
      ".ad-perm-pop{position:fixed;right:22px;top:calc(50% + 30px);width:300px;max-height:60vh;overflow:auto;background:var(--dsw-alias-bg-layer-2,#fff);border:1px solid color-mix(in srgb,#e8a33d 45%,transparent);border-radius:12px;box-shadow:var(--dsw-shadow-lv3,0 8px 30px rgba(0,0,0,.25));z-index:9998;display:none;padding:6px;font-size:12px;line-height:1.45;backdrop-filter:blur(10px)}" +
      ".ad-perm-pop.visible{display:block}" +
      ...
      ".ad-perm-item .p-desc{color:#8a6a2e;word-break:break-all}" +
      ...
      ".ad-perm-actions{display:flex;flex-wrap:wrap;gap:6px;margin-top:7px}" +
      ...
      ".ad-host-approval-pop{position:fixed;right:22px;top:calc(50% + 110px);width:320px;max-height:70vh;overflow:auto;background:var(--dsw-alias-bg-layer-2,#fff);border:1px solid color-mix(in srgb,#357abd 45%,transparent);border-radius:12px;box-shadow:var(--dsw-shadow-lv3,0 8px 30px rgba(0,0,0,.25));z-index:9998;display:none;padding:6px;font-size:12px;line-height:1.45;backdrop-filter:blur(10px)}" +
      ".ad-host-approval-pop.visible{display:block}" +
      ...
      ".ad-ha-item .ha-reason{color:var(--dsw-alias-label-secondary,inherit);font-size:11px;word-break:break-all}" +
      ".ad-ha-item .ha-paths{color:var(--dsw-alias-label-tertiary,inherit);font-size:10px;font-family:var(--ad-mono,monospace);word-break:break-all;max-height:60px;overflow:auto}" +
      ".ad-ha-item .ha-cwd{color:var(--dsw-alias-label-tertiary,inherit);font-size:9.5px;font-family:var(--ad-mono,monospace)}" +
      ".ad-ha-actions{display:flex;flex-wrap:wrap;gap:5px;margin-top:6px}" +
      ...
      return React.createElement("div", {
        ...
        a.permissionPending
          ? React.createElement("span", {
              ...
              title: "ACP 权限待授权：" + (a.permissionPending.description || "") + "\n请点击卡片打开该子代理会话界面，在弹窗中点击允许/拒绝",
            },
            "⏳ 待授权" + (a.startedAt ? " " + fmtDur(a.startedAt, now) : ""))
    ...
    function AgentPanel() {
      ...
      return React.createElement("div", { className: "ad-panel" },
        React.createElement("div", { className: "ad-panel-head" },
          ...
          act.active.filter((a) => a && a.permissionPending).length > 0
            ? React.createElement("span", {
              ...
              "⏳ " + act.active.filter((a) => a && a.permissionPending).length + " 待授权")
            : null,
        ),
      ...
      const poll = () => {
        fetch("/agent-api/active")
          ...
          .then((d) => {
            ...
            const pendingCount = arr.filter((a) => a && a.permissionPending).length;
    ...
    function mountPermFab() {
      ...
      // 渲染列表：每个 pending 子代理一项；信息行点击跳转其会话，操作行提供
      // 授权决策按钮（v1.9.0）：会话期总是允许 / 总是允许 / 拒绝 / 前往授权
      const sendDecision = (a, answer, label) => {
        const btn = (a._btn) || null;
        if (btn) { btn.disabled = true; btn.textContent = "处理中…"; }
        apiPost("/agent-api/permission-decision", { childId: a.childId, answer })
          .then(() => {
      ...
      const renderPop = () => {
        ...
        for (const a of lastPending) {
          ...
          info.title = "打开该子代理会话界面（审批弹窗在那里也可操作）";
          ...
          desc.textContent = (a.permissionPending.description || "请求权限").slice(0, 160);
          ...
          info.addEventListener("click", () => {
            ...
            Promise.resolve(r).then((ok) => {
              if (!ok) {
                ...
                tip.textContent = "无法自动跳转（子代理会话未就绪）。请从左侧会话树或 Agent 面板「运行中」打开该子代理会话。";
                pop.appendChild(tip);
              }
            });
          });
          ...
          const mkBtn = (label, answer, cls, tip) => {
          ...
          mkBtn("本会话总是允许", "allow-session", "", "仅本次主代理会话内生效（内存，会话结束即失效）");
          mkBtn("总是允许(项目)", "allow-always", "", "落盘到用户授权白名单，同工作目录跨会话生效");
      ...
      const poll = () => {
        apiGet("/agent-api/active")
          .then((d) => {
            ...
            const pending = arr.filter((a) => a && a.permissionPending);
            lastPending = pending; // 供 renderPop 取最新（ball click 时渲染）
            // v1.11.4：琥珀覆盖集合同步给蓝球孪生去重（琥珀优先）
            try { acpTwinSync(new Set(pending.map((a) => String((a && a.childId) || "")))) } catch { /* 不影响轮询 */ }
            if (pending.length > 0) {
              ball.classList.add("visible");
              count.textContent = pending.length > 99 ? "99+" : String(pending.length);
    ...
    const acpTwinHold = new Map() // sessionKey -> { item, visible, timer }
    let acpTwinLastAmberKeys = new Set()
    let acpTwinBlueOps = null // mountHostApprovalFab 注入 { add, remove }
    const acpTwinPromote = (key) => {
      const held = acpTwinHold.get(key)
      if (!held || held.visible || !acpTwinBlueOps) return
      held.visible = true
      clearTimeout(held.timer)
      acpTwinBlueOps.add(held.item)
    }
    const acpTwinSync = (amberKeys) => {
      // 琥珀出现 → 隐藏蓝球上已显示的孪生条目
      for (const [key, held] of acpTwinHold.entries()) {
        if (held.visible && amberKeys.has(key) && acpTwinBlueOps) {
          held.visible = false
          acpTwinBlueOps.remove(key)
        }
      }
      // 琥珀消失（且未决议）→ 恢复蓝球兜底显示
      for (const key of acpTwinLastAmberKeys) {
        if (!amberKeys.has(key)) acpTwinPromote(key)
      }
    ...
    // 提供四个按钮：允许一次 / 本会话总是允许 / 总是允许(项目) / 拒绝。
    ...
    function mountHostApprovalFab(clientCtx) {
        ...
        for (const item of pendingApprovals) {
          ...
          mkBtn("本会话总是允许", async () => {
            try {
              ...
              console.warn("[agent-dispatch] 写会话规则失败（仍放行本次）:", (e && e.message) || e);
            }
            await item.answer("allowed-once");
            removeItem(item);
          }, "", "本次会话内同类请求自动放行");
      ...
      acpTwinBlueOps = {
        add(item) {
          if (pendingApprovals.includes(item)) return;
          pendingApprovals.push(item);
          updateBall();
          if (pop.classList.contains("visible")) renderPop();
        },
        remove(key) {
          const idx = pendingApprovals.findIndex((i) => i._twinKey === key);
          if (idx >= 0) pendingApprovals.splice(idx, 1);
          updateBall();
          if (pop.classList.contains("visible")) renderPop();
        },
      ...
      try {
        disposeRemote = remote.$on("approval/request", function(request, next) {
          ...
          if (isAcpTwin && sessionId) {
            ...
            if (!acpTwinHold.has(twinKey)) {
              const held = { item, visible: false, timer: null };
              held.timer = setTimeout(() => {
                // 琥珀 6s（2 个轮询周期）内未覆盖 → 蓝球兜底显示，避免请求丢失
                if (!acpTwinLastAmberKeys.has(twinKey)) acpTwinPromote(twinKey);
              }, 6000);
              item._twinKey = twinKey;
              acpTwinHold.set(twinKey, held);
              const detach = () => { clearTimeout(held.timer); acpTwinHold.delete(twinKey); };
              if (request.signal) {
                const onAbort = () => {
                  if (held.visible && acpTwinBlueOps) acpTwinBlueOps.remove(twinKey);
                  detach();
      ...
      return () => {
        disposeRemote();
        acpTwinBlueOps = null;
        for (const [, held] of acpTwinHold.entries()) clearTimeout(held.timer);
        acpTwinHold.clear();
        ball.remove();
        pop.remove();
      };
    ...
    module.exports = {
      ...
      apply(ctx) {
        ...
        openAgentSession = (id, parentSessionId) => {
          ...
          if (!sessions) { console.warn("[agent-dispatch] openAgentSession: sessions 服务不可用"); return Promise.resolve(false); }
          // v1.8.3：子代理会话打开须满足宿主 catalog 前置——
          ...
          //      mode:'continuable'（agent_dispatch 创建的子代理均为 continuable）。
