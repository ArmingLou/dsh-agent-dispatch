    ...
    function mountPermFab() {
      ...
      const renderPop = () => {
        ...
        for (const a of lastPending) {
          ...
          desc.textContent = (a.permissionPending.description || "请求权限").slice(0, 120);
          const go = document.createElement("span");
          go.className = "p-go";
          go.textContent = "前往授权 →";
          row.appendChild(name); row.appendChild(desc); row.appendChild(go);
          row.title = "打开该子代理会话界面，在弹窗中点击允许/拒绝";
          row.addEventListener("click", () => {
            const r = openAgentSession ? openAgentSession(a.childId, a.parentSessionId) : Promise.resolve(false);
            // 跳转异步完成（openSubagent 需先 refresh catalog）：成功→收起列表；
            // 失败→pop 内给出可见指引（不再静默），用户可手动从会话树打开
            Promise.resolve(r).then((ok) => {
              if (ok) { hidePop(); return; }
